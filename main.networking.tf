...
module "fw_pip" {
  ...
  location            = azurerm_resource_group.this.location
  name                = var.name_prefix != null ? "${var.name_prefix}-pip-fw-${var.workload_code}-${lower(var.location)}-${var.instance_suffix}" : "ai-alz-pip-fw"
  resource_group_name = var.firewall_definition.resource_group_name != null ? var.firewall_definition.resource_group_name : azurerm_resource_group.this.name
  enable_telemetry    = var.enable_telemetry
...
module "app_gateway_waf_policy" {
  ...
  version = "0.2.0"
  count   = try(var.waf_policy_definition.policy_settings.enabled, true) ? 1 : 0
...
module "application_gateway" {
  ...
  resource_group_name                = azurerm_resource_group.this.name
  app_gateway_waf_policy_resource_id = try(module.app_gateway_waf_policy[0].resource_id, null)

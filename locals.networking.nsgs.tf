...
locals {
  ...
  nsg_name = try(var.nsgs_definition.name, null) != null ? var.nsgs_definition.name : (var.name_prefix != null ? "${var.name_prefix}-nsg-${var.workload_code}-${lower(var.location)}-${var.instance_suffix}" : "ai-alz-nsg")
  nsg_rules = merge(
